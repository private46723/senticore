...
export default function Home() {
  ...
  const platforms = [
    { 
      ...
      icon: <Shield className={cn("w-5 h-5 transition-colors", activePlatformTab === 'soc' ? "text-yellow-400" : "text-zinc-400")} />,
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black text-white font-body selection:bg-primary/30">
              ...
              <div className="hidden lg:flex items-center gap-8 text-[15px] font-medium">
                {navItems.map((item) => (
                  <button 
                    ...
                    className="hover:text-primary transition-colors py-2 relative text-[13px] tracking-wide uppercase font-black text-zinc-100"
                  >
                    {item.name}
                  </button>
                ))}
        ...
        <main className="flex-grow flex items-center pb-20 overflow-hidden relative z-10">
          <div className="container mx-auto px-10 max-w-[1400px]">
            <div className="max-w-5xl">
              ...
              <p className="text-xl md:text-2xl text-zinc-100 max-w-4xl leading-relaxed mb-12 font-medium">
      ...
      {/* About Section */}
      <section id="about" className="bg-black py-32 border-t border-white/10 relative overflow-hidden scroll-mt-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            ...
            <div className="space-y-12">
              <div className="space-y-6">
                ...
                <p className="text-zinc-100 text-lg leading-relaxed font-normal">Senticore was founded to bridge the gap between complex security data and actionable defense. We provide the elite expertise required to manage the modern threat landscape.</p>
              </div>
              <div className="grid grid-cols-2 gap-12 pt-8 border-t border-white/5">
                <div className="space-y-3"><div className="text-4xl font-black text-white italic tracking-tighter">Global</div><div className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em]">Operational Reach</div></div>
                <div className="space-y-3"><div className="text-4xl font-black text-white italic tracking-tighter">24/7</div><div className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em]">Always-On Defense</div></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-10 max-w-[1400px]">
          <div className="flex flex-wrap items-center gap-12 border-b border-white/10 mb-20">
            {platforms.map((platform) => (
              <button key={platform.id} onClick={() => setActivePlatformTab(platform.id)} className={cn("flex items-center gap-4 pb-6 transition-all relative group", activePlatformTab === platform.id ? "opacity-100" : "opacity-60 hover:opacity-100")}>
                {platform.icon} <span className={cn("text-lg font-bold transition-colors", activePlatformTab === platform.id ? "text-white" : "text-zinc-400")}>{platform.name}</span>
          ...
          <div key={activePlatformTab} className="animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              <div className="space-y-12">
                <p className="text-zinc-200 text-xl md:text-[18px] leading-relaxed max-w-xl font-normal">{currentPlatform.description}</p>
                <div className="flex flex-wrap gap-20">
                  {currentPlatform.stats.map((stat, i) => (
                    <div key={i} className="space-y-2"><div className="text-5xl md:text-6xl font-bold text-white">{stat.value}</div><div className="text-xs font-bold text-zinc-400 tracking-widest uppercase">{stat.label}</div></div>
                  ))}
                </div>
                <Button onClick={() => scrollToSection('contact')} className={cn("rounded-full px-10 h-14 font-black uppercase text-sm tracking-widest border-none !text-black", currentPlatform.btnColor)}>{currentPlatform.cta} <ArrowRight className="w-5 h-5 ml-2" /></Button>
              </div>
          ...
          <div className="mt-32 pt-20 border-t border-white/5">
            <h4 className="text-3xl font-bold mb-12 text-white">Specialized <span className="text-primary italic">Security Capabilities</span></h4>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {specializedServices.map((service, i) => (
                <div key={i} className="bg-white/5 border border-white/10 p-8 rounded-xl hover:border-primary/50 transition-all group">
                  ...
                  <h5 className="text-xl font-bold mb-3 text-white">{service.title}</h5>
      ...
      <section id="proven-success" className="bg-black border-t border-white/5 relative overflow-hidden scroll-mt-20">
        <div className="w-full relative aspect-video md:aspect-[21/9] overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col justify-center items-center text-center p-10">
             <div className="max-w-4xl">
                <h2 className="text-5xl md:text-[80px] font-black leading-none tracking-tight uppercase mb-8 text-white">Proven <span className="text-primary italic">Success</span></h2>
                <p className="text-xl md:text-2xl text-white font-medium">Real-time threat visibility across a global infrastructure.</p>
             </div>
          </div>
      ...
      <section id="intel" className="bg-black py-24 border-t border-white/5 scroll-mt-20">
        <div className="container mx-auto px-10 max-w-[1400px]">
          <div className="flex justify-between items-end mb-16">
            <h2 className="text-5xl font-bold text-white">Global Security <span className="text-primary italic">Perspectives</span></h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
            <div className="space-y-12">
              <div className="space-y-6">
                ...
                <p className="text-zinc-400 text-lg leading-relaxed max-w-lg">Our experts are available to discuss your SOC requirements, EDR strategy, or emergency incident response needs.</p>
              </div>
              ...
              <div className="space-y-8 pt-8 border-t border-white/5">
                <div className="flex items-center gap-6 group">
                  ...
                  <div>
                    <div className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-1">Email Operations</div>
                ...
                <div className="flex items-center gap-6 group">
                  ...
                  <div>
                    <div className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-1">LinkedIn</div>
            ...
            <div className="bg-[#121212] p-10 rounded-2xl border border-white/5 shadow-2xl relative overflow-hidden">
               ...
               <form className="relative z-10 space-y-6" onSubmit={(e) => e.preventDefault()}>
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Enterprise Email</label>
                      <Input type="email" placeholder="john@company.com" className="bg-black/50 border-white/10 text-white h-12 focus:border-primary" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Security Inquiry</label>
                  ...
                  <Button className="w-full bg-primary hover:bg-primary/90 text-white font-black uppercase tracking-widest h-14 rounded-full shadow-lg flex items-center justify-center gap-3 border-none">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-2 lg:offset-1">
              ...
              <ul className="space-y-4 text-[13px]">
                ...
                <li><a href="#" className="hover:text-primary transition-colors flex items-center gap-2"><Instagram className="w-4 h-4" /> Instagram</a></li>
                <li><a href="mailto:contact@senticore.com" className="hover:text-primary transition-colors flex items-center gap-2"><Mail className="w-4 h-4" /> Email Us</a></li>
              </ul>
            </div>
          </div>
          <div className="mt-24 pt-10 border-t border-white/10 flex items-center justify-between">
            <SenticoreLogo className="w-10 h-10" />
            <p className="text-[11px] text-zinc-500 font-medium uppercase tracking-widest">© 2025 Senticore Security Global. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
